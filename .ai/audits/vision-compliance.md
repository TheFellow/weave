...
lexical adapter, companion adapter packaging, and repository-scale hardening
recorded in this file, plus the workspace/structured-content increment.
...
| Compiler-native semantic truth with visible evidence/provider | **Partial** | Go uses `go/packages`/`go/types`; C# uses Roslyn/MSBuild; F# uses FCS; Python uses CPython `ast`/`symtable` with exact lexical, declared import, and syntactic call evidence | Visual Basic and other languages absent; F#/Python type-enriched call coverage absent; syntax fallback absent. |
...
| Cross-platform core and isolated native adapters | **Partial** | Pure-Go core; release build matrix for macOS/Linux/Windows amd64/arm64; one-shot adapter protocol; .NET 10 CI/packages on all three OSes; Python installed-wheel/query smoke matrix configured for all three OSes; genuine `fkyeah` indexing | Core-only behavior is not tested on a three-OS CI matrix; no tagged release has exercised publication. |
...
3. Define an explicit producer/freshness lifecycle for imported SCIP and
