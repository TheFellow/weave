...
Audited implementation: `main` through the query-driven adapter, federated
freshness, and change-impact block recorded in this file.
...
| Global catalog and bounded federation | **Partial** | catalog add/remove/list/status/sync; refresh-before-open, stale-member exclusion, exact-ID bounded federation, provenance and partial-failure tests | No checked-in cross-repository link configuration; external-symbol resolution is exact-ID only. |
...
| 2 | Enter Go, C#, or F# repository | **Partial** | Go is built in. C#/F# is automatic once a separately built `weave-dotnet` is on `PATH` or configured; it is not in release archives. |
...
The blocking differences are an actually published/tagged install artifact with
the .NET adapter, finer-grained .NET refresh, and unmanaged explicit SCIP import
currency. The automatic installed-adapter and catalog lifecycle is now usable,
but is not equivalent to shipping one executable for all three languages.

## Deferred work in priority order

1. Run and fix the first prerelease publication; package the .NET adapter, then
   add core three-OS CI,
...
3. Add declared/generated cross-language and cross-repository bridges.
...
5. Measure incremental refresh and database size on the four named real
