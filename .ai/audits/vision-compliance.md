...
arbitrary-adapter registration, companion adapter packaging, repository-scale
hardening, the workspace/content increment, contextual link authoring, and
focused DOT graph export.
...
| Compiler-native semantic truth with visible evidence/provider | **Partial** | Go uses `go/packages`/`go/types`; C# uses Roslyn/MSBuild; F# uses FCS; Python uses CPython `ast`/`symtable`; Rust uses `rust-analyzer scip`; C/C++/CUDA use `scip-clang`; TypeScript/JavaScript use `scip-typescript`; Java/Kotlin use `scip-java` | Visual Basic, Scala, and many ecosystems remain absent; F#/Python type-enriched call coverage and a labeled syntax fallback are absent. |
...
| Cross-platform core and isolated native adapters | **Partial** | Pure-Go core; release matrix for macOS/Linux/Windows; .NET/Python/Rust contract matrices on all three OSes; Go wrappers for C++, TypeScript, and JVM are tested on all three; real Linux producer E2E jobs; JVM wrapper build/list/doctor/unit tests do not require Java; genuine `fkyeah` indexing | scip-clang has no upstream Windows or macOS x86-64 binary; producer runtime availability still follows each ecosystem; no tagged release has exercised publication. |
